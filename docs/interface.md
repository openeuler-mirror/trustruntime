...
- 服务端最大空闲时间60秒，连接无数据传输超过此时长后主动断开
...
| 4 | CRL吊销 | 0x13、0x15 |
| 5 | 签名不匹配 | 0x13、0x15 |
| 6 | 证书KeyUsage无效 | 0x13、0x15 |
| 7 | 格式错误（CMS DER解析失败） | 0x13、0x15 |
| 10 | 证书加载失败 | 0x11、0x13、0x15 |
| 12 | 签名算法错误 | 0x11、0x13 |
...
- **result=1（其他节点签名）**：
  - 三次请求（0x10,0x12,0x14）分别在三个节点上完成
  - 首次签名请求和最后一次验签请求不是同一节点
  - 这是正常的多节点协作场景

- **result=2（证书身份冲突）**：
  - 三次请求（0x10,0x12,0x14）均在一个节点上完成
  - 这是安全告警，表示可能存在身份冒用或配置错误
  - 需要检查证书配置是否正确
...
**流程**：验证 signed_data 是否为 sign(data + 输入 id) → 验签通过后判断证书身份 → 返回 result。验签时仅忽略签名方证书过期错误，CA证书过期和证书尚未生效时验签失败。证书过期由 cert_checker 定时巡检 warn。
...
| 验签时签名方证书过期 | — | 仅忽略签名方证书过期错误，验签通过（result 返回 0/1/2）；CA证书过期和证书尚未生效时验签失败 |
