...
- **路径**: `/etc/cert/server/`
  ...
  - CRL：`cert.crl`（可选）
- **CRL**：`cert.crl`（可选）
- **CRL过期处理（启动时）**：过期则vsock启动失败
- **CRL过期处理（运行中）**：记录警告日志，拒绝客户端证书
...
- **CRL**：`/etc/cert/cms/cms.crl`
- **CRL过期处理（启动时）**：过期则插件初始化失败
- **CRL过期处理（运行中）**：记录警告日志，返回验签错误（result=4）
...
| 8 | 私钥不可用 | 0x11、0x13 |
| 9 | 签名算法错误 | 0x11、0x13 |
| 20 | JSON解析错误 | 0x13、0x15 |
| 21 | Base64解码错误 | 0x13、0x15 |
