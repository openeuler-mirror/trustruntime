...
/// CRL有效期（天）
pub const CRL_VALIDITY_DAYS: u32 = 365;

/// 证书版本
pub const CERT_VERSION: i32 = 2;
...
pub fn create_ca_cert(group: &EcGroup, cn: &str) -> (X509, PKey<openssl::pkey::Private>, Vec<u8>) {
    ...
    let not_after =
        Asn1Time::days_from_now(CERT_VALIDITY_DAYS).expect("Failed to create not_after");
    builder
...
pub fn generate_crl(
    ...
    let next_update =
        Asn1Time::days_from_now(CRL_VALIDITY_DAYS).expect("Failed to create next_update");
...
pub fn create_empty_crl(ca_cert: &X509, ca_pkey: &PKey<openssl::pkey::Private>) -> Vec<u8> {
    let mut crl_builder = X509CrlBuilder::new().expect("Failed to create CRL builder");
    ...
    crl.to_pem().expect("Failed to PEM encode CRL")
}

pub fn create_crl_with_revoked(
    ca_cert: &X509,
    ca_pkey: &PKey<openssl::pkey::Private>,
    revoked_certs: &[X509],
) -> Vec<u8> {
